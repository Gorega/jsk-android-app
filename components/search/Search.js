...
import { TextInput, TouchableOpacity, View, StyleSheet, Text, ScrollView, SafeAreaView, Platform, Animated, Keyboard, Dimensions } from 'react-native';
...
export default function Search({
  ...
  // Listen for scanned reference ID from scanReference screen via event emitter
  useEffect(() => {
    if (!onScanCollection) return;

    const handleReferenceScan = (scannedId) => {
      if (scannedId) {
    ...
    const unsubscribe = eventEmitter.on(EVENTS.REFERENCE_SCANNED, handleReferenceScan);
  ...
  useFocusEffect(
    useCallback(() => {
      // This is now handled by the event emitter above
      return () => {
  ...
  return <>
    <SafeAreaView style={[styles.searchBox, { paddingTop: addPaddingSpace ? 32 : 12, backgroundColor: colors.card }]}>
      <View style={styles.search}>
        ...
        <View style={styles.actionButtons}>
          ...
          {Array.isArray(groupByOptions) && groupByOptions.length > 0 && (
            <TouchableOpacity
              style={[
                styles.iconButton,
                styles.actionIconButton,
                activeGroupBy && activeGroupBy.length > 0 && styles.activeIconButton
              ]}
              onPress={handleOpenGroupByModal}
            >
              <MaterialCommunityIcons name="format-list-group" size={22} color={activeGroupBy && activeGroupBy.length > 0 ? "#FFFFFF" : "#4361EE"} />
            </TouchableOpacity>
          )}
        </View>
      </View>

      {(activeSearchBy || activeDate || (activeGroupBy && activeGroupBy.length > 0)) && (
        <View style={styles.activeFiltersBar}>
          ...
          {activeGroupBy && activeGroupBy.length > 0 && (
            <View style={styles.activeFilterChip}>
              <Text style={styles.activeFilterChipText}>
                {activeGroupBy.map(g => g.name || g.action).join(' › ')}
              </Text>
              <TouchableOpacity onPress={() => setActiveGroupBy && setActiveGroupBy([])}>
                <Ionicons name="close-circle" size={18} color="#4361EE" />
              </TouchableOpacity>
            </View>
          )}
      ...
      {Array.isArray(groupByOptions) && groupByOptions.length > 0 && (
        <GestureScrollView
          ...
          style={styles.groupByQuickBar}
          contentContainerStyle={styles.groupByQuickBarContent}
        ...
        >
          {groupByOptions.map((item, index) => {
            const isSelected = activeGroupBy?.some(g => g.action === item.action);
            return (
              <GestureTouchableOpacity
                key={item.action || String(index)}
                style={[styles.groupByChip, isSelected && styles.groupByChipActive]}
                activeOpacity={0.7}
                onPress={() => {
                  if (!setActiveGroupBy) return;
                  const next = Array.isArray(activeGroupBy) ? [...activeGroupBy] : [];
                  if (isSelected) {
                    setActiveGroupBy(next.filter(g => g.action !== item.action));
                  } else {
                    next.push(item);
              ...
              >
                <Text style={[styles.groupByChipText, isSelected && styles.groupByChipTextActive]}>
                  {item.name}
                </Text>
              </GestureTouchableOpacity>
            );
    ...
    {showGroupByModal && (
      <ModalPresentation
        showModal={showGroupByModal}
        setShowModal={setShowGroupByModal}
        onDismiss={() => {
          modalTransitionInProgress.current = true;
      ...
      >
        <View style={[styles.modalHeader, { backgroundColor: isDark ? colors.card : 'white', borderBottomColor: isDark ? colors.border : '#E2E8F0' }]}>
          <View style={[styles.modalHeaderBar, { backgroundColor: isDark ? colors.border : '#E2E8F0' }]} />
          <Text style={[styles.modalH2, { color: colors.text }]}>{translations[language]?.search?.groupBy || 'Group By'}</Text>
        </View>
            ...
            return (
              <TouchableOpacity
                ...
                style={[
                  ...
                  { borderBottomColor: isDark ? colors.border : '#E2E8F0', backgroundColor: isSelected ? (isDark ? 'rgba(67, 97, 238, 0.15)' : 'rgba(67, 97, 238, 0.05)') : 'transparent' }
                ]}
                onPress={() => {
                  if (!setActiveGroupBy) return;
                  const next = Array.isArray(activeGroupBy) ? [...activeGroupBy] : [];
                  if (isSelected) {
                    const filtered = next.filter(g => g.action !== item.action);
                    setActiveGroupBy(filtered);
                  } else {
                    next.push(item);
                    setActiveGroupBy(next);
                  }
              ...
              >
                <Text style={[
                  ...
                  { color: isSelected ? '#4361EE' : colors.text }
                ]}>
                  {item.name}
                </Text>
                {isSelected && (
        ...
        <View style={[styles.modalFooter, { backgroundColor: isDark ? colors.card : 'white', borderTopColor: isDark ? colors.border : '#E2E8F0' }]}>
          ...
          <TouchableOpacity
            style={[styles.modalButton, styles.actionIconButton, { backgroundColor: '#4361EE' }]}
            onPress={() => {
              modalTransitionInProgress.current = true;
              setShowGroupByModal(false);
              setTimeout(() => {
          ...
          >
            <Text style={[styles.secondaryButtonText, { color: '#FFFFFF' }]}>{translations[language]?.common?.confirm || 'Confirm'}</Text>
          </TouchableOpacity>
        </View>
      </ModalPresentation>
    )}
...
const styles = StyleSheet.create({
  ...
  groupByQuickBar: {
    marginTop: 6,
    marginHorizontal: -12, // Offset searchBox padding for edge-to-edge scroll
  },
  groupByQuickBarContent: {
    paddingHorizontal: 12,
    paddingBottom: 6,
    flexDirection: 'row',
    alignItems: 'center',
