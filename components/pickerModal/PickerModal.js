...
export default function PickerModal({
    ...
    const effectiveShowPickerModal = typeof visible === 'boolean' ? visible : showPickerModal;
    const effectiveSetShowModal = typeof onClose === 'function' ? onClose : (typeof setShowModal === 'function' ? setShowModal : (() => { }));
    const effectiveList = Array.isArray(data) ? data : initialList;
    const effectiveField = field || { name: 'value', label: title, showSearchBar: true };
    const { name } = effectiveField;
    const [modalHeight, setModalHeight] = useState(Dimensions.get('window').height * 0.7);
    const isRTL = language === 'ar' || language === 'he';
    const insets = useSafeAreaInsets();
    const displayTitle = (effectiveField && effectiveField.label) || title || translations[language].picker.selectOption;
    const displaySearchPlaceholder = searchPlaceholder || translations[language].picker.searchPlaceholder;
    ...
    const displayList = useApiSearch ? internalList : effectiveList;
    ...
    return (
        <Modal
            animationType="slide"
            visible={effectiveShowPickerModal}
                                    ...
                                    <TouchableOpacity
                                        ...
                                        onPress={() => {
                                            if (typeof onSelect === 'function') {
                                                onSelect(item);
                                            } else if (typeof setSelectedValue === 'function') {
                                                setSelectedValue((prevValue) => {
                                                    const newValue = { ...prevValue };
                                                    newValue[name] = item;
                                                    return newValue;
