...
export default function AddOptionsModal({ visible, onClose, userRole }) {
  ...
  const { user } = useAuth();
  const canCreateMultiple = userRole === "business";
  const createMultipleLabel = translations[language]?.tabs?.createMultiple?.title || (language === "ar" ? "إنشاء عدة طلبات" : language === "he" ? "יצירת מספר הזמנות" : "Create Multiple");
                ...
                <>
                  ...
                  {canCreateMultiple && (
                    <TouchableOpacity
                      ...
                      onPress={() => handleOptionPress("/(create_multi)/create-multiple")}
                    >
                      <LinearGradient
                        colors={[colors.gradientStart, colors.gradientEnd]}
                        style={styles.optionGradient}
