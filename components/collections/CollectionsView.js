...
const CollectionItem = React.memo(function CollectionItem({ item, type, isSelectionMode, isSelected, onSelect, onLongPress, highlightCollectionId }) {
    const { colorScheme } = useTheme();
    const colors = Colors[colorScheme];
    const isHighlighted = highlightCollectionId && String(item.collection_id) === String(highlightCollectionId);
    return (
        <View style={styles.order}>
            <TouchableOpacity
                onLongPress={() => onLongPress && onLongPress(item.collection_id)}
                onPress={() => (isSelectionMode ? (onSelect && onSelect(item.collection_id)) : null)}
            ...
            >
                ...
                <View style={{ flex: 1 }} pointerEvents={isSelectionMode ? 'none' : 'auto'}>
                    <Collection type={type} collection={item} highlight={isHighlighted} />
                </View>
            </TouchableOpacity>
        </View>
    );
    ...
    return prevProps.item.collection_id === nextProps.item.collection_id && 
           ...
           prevProps.highlightCollectionId === nextProps.highlightCollectionId;
});

export default function CollectionsView({data, type, loadMoreData, loadingMore, refreshControl, isLoading, isSelectionMode = false, selectedCollectionIds = [], onSelectCollection, onLongPressCollection, listRef, highlightCollectionId, onScrollToIndexFailed}){
    ...
    const renderCollectionItem = React.useCallback(({ item }) => {
        return (
            <CollectionItem
                ...
                isSelected={selectedCollectionIds.includes(item.collection_id)}
                onSelect={onSelectCollection}
                onLongPress={onLongPressCollection}
                highlightCollectionId={highlightCollectionId}
            />
        );
    }, [type, isSelectionMode, selectedCollectionIds, onSelectCollection, onLongPressCollection, highlightCollectionId]);
