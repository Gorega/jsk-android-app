...
const OrderItem = React.memo(function OrderItem({ item, metadata, onStatusChange, isSelectionMode, isSelected, onSelect, onLongPress, onExplicitSelect, isSelectionClickGuard, initialMinimized }) {
    const { colorScheme } = useTheme();
    const colors = Colors[colorScheme];
    return (
        <View style={styles.orderContainer}>
            <TouchableOpacity
                onLongPress={() => onLongPress && onLongPress(item.order_id)}
                onPress={() => (isSelectionMode && !isSelectionClickGuard) ? onSelect && onSelect(item.order_id) : null}
            ...
            >
                ...
                <View style={{ flex: 1 }} pointerEvents={isSelectionMode ? 'box-none' : 'auto'}>
                    <Order 
                        user={metadata} 
                        order={item} 
                        ...
                        onSelect={onSelect}
                        onExplicitSelect={(orderId, shouldSelect) => onExplicitSelect && onExplicitSelect(orderId, shouldSelect)}
    ...
    return prevProps.item.order_id === nextProps.item.order_id &&
        ...
        prevProps.initialMinimized === nextProps.initialMinimized &&
        JSON.stringify(prevProps.metadata) === JSON.stringify(nextProps.metadata);
});

const areGroupedDataEqual = (left, right) => {
    ...
    for (let i = 0; i < left.length; i += 1) {
        ...
        if (leftItem?.category_name !== rightItem?.category_name) return false;
        if (leftItem?.count !== rightItem?.count) return false;
        const leftOrders = leftItem?.orders;
        const rightOrders = rightItem?.orders;
        if (leftOrders === rightOrders) continue;
        const leftIsArray = Array.isArray(leftOrders);
        const rightIsArray = Array.isArray(rightOrders);
        if (leftIsArray !== rightIsArray) return false;
        if (!leftIsArray) continue;
        if (leftOrders.length !== rightOrders.length) return false;
        for (let j = 0; j < leftOrders.length; j += 1) {
            const leftOrder = leftOrders[j];
            const rightOrder = rightOrders[j];
            if (leftOrder?.order_id !== rightOrder?.order_id) return false;
            if (leftOrder?.status_key !== rightOrder?.status_key) return false;
...
const CategorySection = React.memo(({ item, drag, isActive, toggleCategory, isExpanded, renderOrderItem, colors, styles, language, loading, canLoadMore, onLoadMore }) => {
    return (
        <ScaleDecorator>
            <View>
                <TouchableOpacity
                ...
                {isExpanded && (
                    <View>
                        {loading && (!item.orders || item.orders.length === 0) ? (
...
const OrdersView = React.memo(function OrdersView({
    ...
    categoryLoadingMap = {},
    categoryPagination = {},
    onFetchCategoryOrders,
    onLoadMoreCategoryOrders,
    isSelectionMode,
    selectedOrderIds,
    onSelectOrder,
    onLongPressOrder,
    onExplicitSelectOrder,
    isSelectionClickGuard,
    initialMinimized
}) {
    const { language } = useLanguage();
    const { colorScheme } = useTheme();
    ...
    const toggleCategory = React.useCallback((categoryItem) => {
        setExpandedCategories(prev => {
            const newSet = new Set(prev);
            const categoryName = categoryItem.category_name;
            const isExpanding = !newSet.has(categoryName);
            if (isExpanding) {
                newSet.add(categoryName);
                if (onFetchCategoryOrders && categoryItem.groupItem) {
                    const existing = categoryOrdersMap[categoryName];
                    if (!Array.isArray(existing) || existing.length === 0) {
                        onFetchCategoryOrders(categoryItem.groupItem);
                    }
                }
            } else {
    ...
    }, [categoryOrdersMap, onFetchCategoryOrders]);
    ...
    useEffect(() => {
        if (!["driver", "delivery_company"].includes(userRole)) {
            setGroupedData(prev => (prev.length === 0 ? prev : []));
            return;
        }

        const UNCATEGORIZED_KEY = '__UNCATEGORIZED__';
        const useCategoryGroups = Array.isArray(categoryGroups) && categoryGroups.length > 0;

        if (useCategoryGroups) {
            const normalizedGroups = categoryGroups.map(group => {
                const rawValue = group?.group_value;
                const catKey = rawValue === null || rawValue === undefined || rawValue === '' ? UNCATEGORIZED_KEY : String(rawValue);
                return {
                    category_name: catKey,
                    display_name: group?.group_value_label || (rawValue != null ? String(rawValue) : ''),
                    count: group?.total_orders || group?.order_count || (Array.isArray(group?.order_ids) ? group.order_ids.length : 0),
                    orders: categoryOrdersMap[catKey] || [],
                    groupItem: group,
                    key: `cat-${catKey}`
                };
            });

            const categoryNames = normalizedGroups.map(group => group.category_name);
            ...
            if (localCategoryOrder.length > 0) {
                ...
                orderedCategories = [...categoryNames];
                setLocalCategoryOrder(orderedCategories);
            }

            const newGroupedData = orderedCategories
                .map(cat => normalizedGroups.find(group => group.category_name === cat))
                .filter(Boolean);
            setGroupedData(prev => (areGroupedDataEqual(prev, newGroupedData) ? prev : newGroupedData));
            return;
        }
        ...
        setGroupedData(prev => (areGroupedDataEqual(prev, newGroupedData) ? prev : newGroupedData));
    }, [data, userRole, language, categoryGroups, categoryOrdersMap, localCategoryOrder]);
    ...
    const renderOrderItem = React.useCallback(({ item }) => {
        ...
        return (
            <OrderItem
                ...
                onStatusChange={onStatusChange}
                isSelectionMode={isSelectionMode}
                isSelected={selectedOrderIds && selectedOrderIds.includes(item.order_id)}
                onSelect={onSelectOrder}
                onLongPress={onLongPressOrder}
                onExplicitSelect={(orderId, shouldSelect) => onExplicitSelectOrder && onExplicitSelectOrder(orderId, shouldSelect)}
                isSelectionClickGuard={isSelectionClickGuard}
                initialMinimized={initialMinimized}
            />
        );
    }, [metadata, onStatusChange, toggleCategory, colors, language, isSelectionMode, selectedOrderIds, onSelectOrder, onLongPressOrder, onExplicitSelectOrder, isSelectionClickGuard, initialMinimized]);
    ...
    const getCategoryLoadState = useCallback((categoryItem) => {
        const categoryName = categoryItem?.category_name;
        const orders = Array.isArray(categoryItem?.orders) ? categoryItem.orders : [];
        const pagination = categoryPagination?.[categoryName];
        const total = typeof pagination?.total === 'number'
            ? pagination.total
            : (categoryItem?.count ?? orders.length);
        const limit = typeof pagination?.limit === 'number'
            ? pagination.limit
            : (typeof pagination?.page_size === 'number' ? pagination.page_size : orders.length);
        const page = typeof pagination?.page === 'number' ? pagination.page : 1;
        const loadedCount = pagination ? page * limit : orders.length;
        const canLoadMore = total > loadedCount;
        const loading = !!categoryLoadingMap?.[categoryName];
        return { loading, canLoadMore };
    }, [categoryLoadingMap, categoryPagination]);

    const renderCategoryItem = useCallback(({ item, drag, isActive }) => {
        const { loading, canLoadMore } = getCategoryLoadState(item);
        const handleLoadMore = () => {
            if (!onLoadMoreCategoryOrders || loading || !canLoadMore) {
                return;
            }
            onLoadMoreCategoryOrders(item?.groupItem || item);
        };
        return (
            <CategorySection
                item={item}
    ...
    }, [expandedCategories, getCategoryLoadState, onLoadMoreCategoryOrders, renderOrderItem, toggleCategory, colors, styles, language]);

    // Loading state - show loading indicator when isLoading is true
    const isInitialLoading = isLoading || categoryGroupsLoading;
