...
function Order({ user, order, globalOfflineMode, pendingUpdates, hideSyncUI = true, onStatusChange, onLongPress, isSelectionMode, isSelected, onSelect, onExplicitSelect, initialMinimized }) {
    ...
    const [isMinimized, setIsMinimized] = useState(
        initialMinimized !== undefined
            ? initialMinimized
            : ["business", "driver", "delivery_company", "admin", "manager", "accountant", "entery", "warehouse_admin", "warehouse_staff", "support_agent"].includes(authUser?.role || "")
    );
    ...
    const [expandedPackageItems, setExpandedPackageItems] = useState([]);
    const [isLoadingPackageItems, setIsLoadingPackageItems] = useState(false);
        ...
        return items
            .map((item, index) => {
                ...
                if (typeof item === 'object') {
                    ...
                    const sequence = item.sequence ?? item.seq ?? index + 1;
                    const text = qrId ? String(qrId) : '-';
                    return { key: `${index}-${text}`, text };
                }
    ...
    const packageItemsCount = packageItems.length;
    const quantityValue = parseInt(order?.quantity || order?.number_of_items || 0, 10) || 0;
    const followUpCount = Math.max(packageItemsCount, Math.max(quantityValue - 1, 0));
    const hasFollowUpOrders = followUpCount > 0;
    const displayPackageItems = useMemo(() => {
        if (expandedPackageItems.length > 0) return expandedPackageItems;
        if (packageItemsCount > 0) {
            return packageItems.map((item, idx) => ({
                key: item.key || `${idx}-${item.text}`,
                qr_id: item.text
            }));
        }
    ...
    }, [expandedPackageItems, packageItems, packageItemsCount, followUpCount]);
    ...
    const shouldHighlightHeader = authUserRole !== 'business';
    const headerDividerColor = shouldHighlightHeader
        ? (isJerusalemReceiver ? '#3B82F6' : (is48Receiver ? (is48TerritoryFlagged ? '#EF4444' : '#FACC15') : colors.border))
        : colors.border;
    const headerBackgroundColor = shouldHighlightHeader
        ? (isJerusalemReceiver
            ? (isDark ? 'rgba(59, 130, 246, 0.18)' : 'rgba(59, 130, 246, 0.10)')
            : is48Receiver
                ? (is48TerritoryFlagged
                    ? (isDark ? 'rgba(239, 68, 68, 0.18)' : 'rgba(239, 68, 68, 0.12)')
                    : (isDark ? 'rgba(250, 204, 21, 0.16)' : 'rgba(250, 204, 21, 0.18)'))
                : (isDark ? colors.card : 'rgba(67, 97, 238, 0.05)'))
        : colors.card;
    const receiverLocationMarkColor = is48Receiver ? (is48TerritoryFlagged ? '#EF4444' : '#FACC15') : null;
    ...
    const handleDriverSearch = (query) => {
        ...
        setDriverPage(1);
        setDriverHasMore(true);
        // Debounce handled by PickerModal effects usually, but here we might need to be careful
        // The PickerModal handles debouncing internally for its own search field if we pass setPickerSearchValue
    ...
    const togglePackageItems = async () => {
        const next = !showPackageItems;
        setShowPackageItems(next);
        if (next && expandedPackageItems.length === 0 && followUpCount > 0) {
            try {
                setIsLoadingPackageItems(true);
                ...
                let pkg = [];
                if (Array.isArray(res.data?.package_items)) {
                    pkg = res.data.package_items;
                } else if (typeof res.data?.package_items === 'string') {
                    try {
                        const parsed = JSON.parse(res.data.package_items);
                        pkg = Array.isArray(parsed) ? parsed : [];
                    } catch (e) {
                        pkg = [];
                    }
                }
                const normalized = pkg
                    .filter(x => x && (x.qr_id || x.sequence))
                ...
                if (normalized.length > 0) {
                    setExpandedPackageItems(normalized);
                }
            } catch (e) {
            } finally {
                setIsLoadingPackageItems(false);
            }
        }
    };
    ...
    const processPendingStatusUpdates = async (updates = pendingStatusUpdates, showSuccessMessage = true) => {
        ...
        for (const update of updatesToProcess) {
            try {
                ...
                const requiresDriver = statusOptions.some(option => option.value === update.status && option.requiresDriver);
                const resolvedDriverId = update.driver_id || resolveDriverId(update.driver);
                if (requiresDriver && !resolvedDriverId) {
                    update.errorDetails = translations[language]?.tabs?.orders?.order?.selectDriver || 'Select driver';
                    failedUpdates.push(update);
                    failedOrderIds.push(update.order_id);
                    continue;
                }
                if (!update.driver_id && resolvedDriverId) {
    ...
    const changeStatusHandler = async () => {
        ...
        try {
            setIsUpdating(true);
            const prevStatusLabel = order.status;
            const prevStatusKey = order.status_key;
            ...
            if (selectedStatus === 'in_branch') {
                if (isAdmin && !selectedBranch?.value) {
                    setErrorMessage(translations[language]?.tabs?.orders?.order?.selectBranch || 'Select Branch');
                    setShowErrorModal(true);
                    setIsUpdating(false);
                    return;
                }
            ...
            if (isOnline) {
                ...
                try {
                    ...
                    // Optimistic UI update before sending status change to server
                    if (typeof onStatusChange === 'function') {
                    ...
                    if (thisOrderFailed || data.error) {
                        ...
                        if (didOptimisticUpdate && typeof onStatusChange === 'function') {
                            onStatusChange(order.order_id, prevStatusLabel, prevStatusKey);
                        }
                    } else {
                    ...
                    if (response && typeof response === 'object') {
                        ...
                        setShowErrorModal(true);
                        // Revert optimistic UI on network/server error
                        if (didOptimisticUpdate && typeof onStatusChange === 'function') {
                            onStatusChange(order.order_id, prevStatusLabel, prevStatusKey);
                        }
                    } else {
    ...
    return (
        <RTLWrapper>
            <Pressable
                onPress={() => {
                    if (isSelectionMode) {
                        if (typeof onExplicitSelect === 'function') {
                            onExplicitSelect(order.order_id, !isSelected);
                ...
                <View style={[styles.orderCard, {
                    ...
                    <View style={[styles.header, {
                        ...
                        <View style={[styles.orderIdSection]}>
                            <View style={[styles.orderIdContainer]}>
                                ...
                                {!!(order?.financial_status && String(order.financial_status).trim()) && (
                                    <MaterialIcons
                                        name="check-circle"
                                        size={16}
                            ...
                            <TouchableOpacity
                                style={[styles.orderActionsButton, { borderColor: colors.border }]}
                                onPress={() => setShowControl(true)}
                                activeOpacity={0.7}
                            >
                                <MaterialIcons name="more-horiz" size={18} color={colors.textSecondary} />
                                <Text style={[styles.orderActionsText, { color: colors.textSecondary }]}>
                    ...
                    {isMinimized ? (
                        <View style={[styles.minimizedContainer, {
                            ...
                            {hasFollowUpOrders && (
                                <View style={[styles.minimizedRow]}>
                                    <View style={[styles.minimizedSection, styles.minimizedPackageItemsSection]}>
                                        <TouchableOpacity
                                            activeOpacity={0.7}
                                            onPress={togglePackageItems}
                                            style={styles.packageItemsHeader}
                                        >
                                            <View style={styles.packageItemsHeaderLeft}>
                                                <View style={[
                                                    styles.iconWrapper,
                                                ...
                                                <View style={styles.packageItemsHeaderText}>
                                                    <Text style={[styles.sectionTitle, {
                                                        ...
                                                        {translations[language]?.tabs?.orders?.order?.packageItemsTitle || 'Package items'}
                                                    </Text>
                                                    <Text style={[styles.packageItemsCount, {
                                                        color: colors.text, ...Platform.select({
                                                            ios: {
                                                                textAlign: isRTL ? "left" : ""
                                                            }
                                            ...
                                            <View style={[
                                                styles.packageItemsToggle,
                                                { backgroundColor: showPackageItems ? 'rgba(14, 165, 233, 0.2)' : 'rgba(14, 165, 233, 0.12)' }
                                            ]}>
                                                <Text style={[styles.packageItemsToggleText, { color: colors.text }]}>
                                                    {showPackageItems
                                                        ? (translations[language]?.tabs?.orders?.order?.packageItemsHide || 'Hide')
                                                        : (translations[language]?.tabs?.orders?.order?.packageItemsShow || 'Show')}
                                                </Text>
                                                <MaterialIcons name={showPackageItems ? "expand-less" : "expand-more"} size={18} color={colors.text} />
                                            </View>
                                        </TouchableOpacity>
                                        {showPackageItems && (
                                            <View style={[styles.packageItemsList, { borderTopColor: colors.border }]}>
                                                {isLoadingPackageItems ? (
                                                    <View style={[styles.packageItemRow, { borderBottomColor: colors.border }]}>
                                                        <ActivityIndicator size="small" color={colors.textSecondary} />
                                                    </View>
                                                ) : displayPackageItems.map((item, index) => (
                                                    <View
                                                        key={item.key}
                                                        style={[styles.packageItemRow, { borderBottomColor: colors.border }]}
                                                    >
                                                        <Text style={[styles.packageItemIndex, {
                                                            color: colors.textSecondary, ...Platform.select({
                                                                ios: {
                                                                    textAlign: isRTL ? "left" : ""
                                                                }
                            ...
                            <View style={[styles.minimizedRow]}>
                                <View style={[
                                    ...
                                    <Text style={[styles.minimizedValue, {
                                        color: receiverLocationMarkColor || colors.text, ...Platform.select({
                                            ios: {
                                                textAlign: isRTL ? "left" : ""
                                            }
                                        })
                        ...
                        >
                            ...
                            <View style={[styles.userInfoSection]}>
                                ...
                                {!["driver", "delivery_company"].includes(authUserRole) &&
                                    order.driver &&
                                    order.driver !== translations[language].tabs.orders.order.unknown && (
                                        <UserBox
                                            box={{
                                                label: translations[language].tabs.orders.order.userDriverBoxLabel,
                                                userName: order.driver,
                                ...
                                <View style={[styles.sectionRow]}>
                                    ...
                                    <View style={[styles.sectionContent, {
                                        ...
                                        <Text style={[styles.locationCity, {
                                            color: receiverLocationMarkColor || colors.text
                                        }]}>
                                            {order.receiver_city}
                                        </Text>
                            ...
                            {hasFollowUpOrders && (
                                <View style={[styles.packageItemsSection, { backgroundColor: colors.surface }]}>
                                    <TouchableOpacity
                                        activeOpacity={0.7}
                                        onPress={togglePackageItems}
                                        style={styles.packageItemsHeader}
                                    >
                                        <View style={styles.packageItemsHeaderLeft}>
                                            ...
                                            <View style={styles.packageItemsHeaderText}>
                                                ...
                                                <Text style={[styles.packageItemsCount, { color: colors.text }]}>
                                                    {followUpCount}
                                                </Text>
                                            </View>
                                        </View>
                                        ...
                                        <View style={[styles.packageItemsList, { borderTopColor: colors.border }]}>
                                            {isLoadingPackageItems ? (
                                                ...
                                                <View
                                                    ...
                                                    style={[styles.packageItemRow, { borderBottomColor: colors.border }]}
                                                >
                                                    <Text style={[styles.packageItemIndex, { color: colors.textSecondary }]}>
                                                        {index + 1}.
                                                    </Text>
                                                    <Text style={[styles.packageItemText, { color: colors.text }]}>
                                                        {item.qr_id}
                                                    </Text>
                                                </View>
                                            ))}
            ...
            {showControl && (
                ...
                >
                    ...
                    <View style={styles.controlContainer}>
                        {!["business", "driver", "delivery_company"].includes(authUserRole) && (
                            <TouchableOpacity
                                style={[
                                ...
                                onPress={() => {
                                    ...
                                    if (isSelected) {
                                        onExplicitSelect && onExplicitSelect(order.order_id, false);
                                    } else {
                                        onExplicitSelect && onExplicitSelect(order.order_id, true);
                                    }
                                }}
                            >
                                <View style={[
                                    ...
                                    <MaterialCommunityIcons name={isSelected ? "checkbox-marked" : "checkbox-blank-outline"} size={18} color="#ffffff" />
                                </View>
                                <Text style={[styles.controlText, {
                                    ...
                                    {isSelected ? (translations[language]?.common?.deselect || "Deselect") : (translations[language]?.common?.select || "Select")}
                                </Text>
                            </TouchableOpacity>
                        )}
...
const styles = StyleSheet.create({
    ...
    packageItemsHeaderLeft: {
        flexDirection: 'row',
        alignItems: 'center',
        gap: 12,
        flex: 1,
    },
    packageItemsHeaderText: {
        flex: 1,
    },
    packageItemsCount: {
        fontSize: 13,
