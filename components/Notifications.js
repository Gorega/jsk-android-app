...
export default function NotificationsComponent() {
  ...
  const handleNotificationItemClick = async (notificationId, notificationType, notificationOrderId, metadata, notificationCollectionId) => {
    try {
      ...
      if (
        ...
        (parsedMetadata.notification_kind === 'complaint_message' || parsedMetadata.notification_kind === 'complaint_created' || parsedMetadata.complaint_id)
      ) {
        const complaintId = parsedMetadata.complaint_id;
        if (complaintId) {
          router.push({
      ...
      if (notificationType === "confirmation") {
        ...
        let parsedMetadataForConfirmation = parsedMetadata;
        
        // Handle metadata - could be a string type identifier or a JSON object
        if (parsedMetadataForConfirmation) {
          if (typeof parsedMetadataForConfirmation === 'string') {
            // If metadata is a simple string, use it as the type
            metadataType = parsedMetadataForConfirmation;
            // Try parsing if it's a JSON string
            try {
              parsedMetadataForConfirmation = JSON.parse(parsedMetadataForConfirmation);
              metadataType = parsedMetadataForConfirmation.type || 'generic';
            } catch (e) {
              // If parsing fails, keep the original metadata
              parsedMetadataForConfirmation = parsedMetadataForConfirmation;
            }
          } else if (typeof parsedMetadataForConfirmation === 'object') {
        ...
        setConfirmationModal({
          ...
          orderData: {
            ...
            metadata: parsedMetadataForConfirmation,
      ...
      if (notificationType === "collection" || (parsedMetadata && parsedMetadata.notification_kind === 'collection_closed')) {
        const collectionId = (parsedMetadata && parsedMetadata.collection_id) || notificationCollectionId;
        if (collectionId) {
          const collectionType = getCollectionTypeParam(parsedMetadata?.collection_type_id);
          router.push({
            pathname: "/(collection)",
            params: {
              type: collectionType,
  ...
  const renderNotification = (notification, index) => {
    ...
    >
      <TouchableOpacity 
        ...
        onPress={() => handleNotificationItemClick(notification.notification_id, notification.type, notification.order_id, notification.metadata, notification.collection_id)}
        activeOpacity={0.7}
      >
