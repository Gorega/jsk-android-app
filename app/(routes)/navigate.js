...
export default function RouteNavigate() {
    ...
    // Add state for selected language for WhatsApp message
    ...
    const [companyType, setCompanyType] = useState('taiar'); // Default to 'taiar', can be 'jsk'
    ...
    const generateWhatsAppMessage = (order, messageLanguage = null) => {
        if (!order) return '';
        ...
        const senderName = order.external_sender_name || order.sender_name || '';
        ...
        // For JSK, handle language selection
        if (companyType.toLowerCase() === 'jsk') {
            ...
            const heMessage = `${companyDisplayName} – הודעת משלוח – בדרך אליך\n\n` +
                ...
                `נשמח להודיעך שההזמנה שלך في طريقها אליך 🚗💨\n\n` +
                `👤 שם: ${receiverName}\n` +
                `📍 אזור: ${receiverCity}\n` +
        ...
        if (language === 'ar') {
            return `عزيزي الزبون (${receiverName}), لديك طرد مرسل من ${senderName} بقيمة ${codValue} شيكل, حالياً موجود لدى شركة ${companyDisplayName} للتوصيل, سوف يصلك ${day}, يرجى ارسال الموقع أو العنوان بالتفصيل على هذا الرقم من أجل تسهيل عملية الاستلام, وشكراً لتعاونك.\nملاحظة هامة: ممنوع قياس الملابس والاحذية او تجربة الاجهزة الكهربائية والكريمات وشكرا`;
    ...
    // Modified message handler to show appropriate modal first
    const handleMessage = (phoneNumber, order) => {
        ...
        // For JSK company type, show language selection modal first
        if (companyType.toLowerCase() === 'jsk') {
            setShowLanguageSelectionModal(true);
        } else {
            // For other companies, continue with day selection modal
    ...
    const handleWhatsApp972 = () => {
        ...
        const message = generateWhatsAppMessage(currentOrderForContact, selectedMessageLanguage);
