...
export default function EditReceiverDetailsScreen() {
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            const shouldUseBusinessMessage = user.role === "business" && ["stuck", "reschedule"].includes(orderData?.status_key);
            const successMessage = shouldUseBusinessMessage
                ? translations[language].tabs.orders.create.businessReceiverDetailsUpdateSuccess
                : translations[language].tabs.orders.create.receiverDetailsUpdateSuccess;
    ...
    if (user?.role === 'business' && ["stuck", "reschedule"].includes(orderData?.status_key)) {
        sections.push({
