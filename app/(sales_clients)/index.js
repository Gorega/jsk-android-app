...
import UsersView from '../../components/sales_clients/UsersView';
...
export default function HomeScreen() {
    ...
    const filterByGroup = useMemo(() => [], [language]);
    ...
    const searchByDateGroup = useMemo(() => [], [language]);
    ...
    const fetchData = useCallback(async (pageNumber = 1, isLoadMore = false) => {
        ...
        try {
            ...
            if (!activeSearchBy && searchValue) queryParams.append('np', searchValue);
            if (activeSearchBy && searchValue) queryParams.append(activeSearchBy.action, searchValue);
            queryParams.append('page', pageNumber);
            queryParams.append('language_code', language);
            
            const res = await fetch(`${process.env.EXPO_PUBLIC_API_URL}/api/sales_rep_clients?${queryParams.toString()}`, {
                method: "GET",
                credentials: "include",
                headers: {
