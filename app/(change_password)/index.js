...
export default function ChangePasswordScreen() {
  ...
  const targetUserIdParam = Array.isArray(params?.userId) ? params.userId[0] : params?.userId;
  const targetUserNameParam = Array.isArray(params?.userName) ? params.userName[0] : params?.userName;
  const effectiveUserId = String(targetUserIdParam || user?.userId || user?.user_id || "");
  const isSelf = Number(effectiveUserId) === Number(user?.userId || user?.user_id);
  ...
  const handleSubmit = async () => {
    ...
    if (validateForm()) {
      ...
      try {
        const response = await fetch(`${process.env.EXPO_PUBLIC_API_URL}/api/users/${effectiveUserId}/update-password`, {
          method: "PUT",
          headers: {
  ...
  return (
    <SafeAreaView style={[styles.safeArea, { backgroundColor: colors.card }]}>
      ...
      <View style={[styles.container, { backgroundColor: colors.background }]}>
        ...
        >
          {!isSelf && effectiveUserId ? (
            <View style={[styles.securityTipsContainer, {
              backgroundColor: isDark ? colors.cardDark : "#F8FAFC",
