...
export default function HomeScreen() {
    ...
    useEffect(() => {
        ...
        const businessRole = roles.find(r => r?.role_id === 2) || roles.find(r => r?.role_name === "business");
        if (!businessRole) return;

        setSelectedValue(prev => ({
            ...prev,
            role_id: businessRole
        }));
        setForm(prev => ({
            ...prev,
            role_id: 2,
            role: businessRole?.name || prev.role
        }));
    }, [roles, user?.role]);
    ...
    const createSender = async (url, method) => {
        ...
        try {
            const roleId = user?.role === "business" ? 2 : (selectedValue.role_id?.role_id || form.role_id);
            const branchId = selectedValue.branch_id?.branch_id || form.branch_id || (user?.role === "business" ? (user.branch_id || user.current_branch_id) : undefined);
            const pricelistId = selectedValue.pricelist_id?.pricelist_id || form.priceList_id || 1;

            const defaultBusinessPermissions = [
                'view_dashboard', 'view_orders', 'view_collections', 'view_money_collections',
                'view_returned_collections', 'view_sent_collections', 'view_business_users',
                'view_receivers', 'view_complaints', 'create_orders', 'export_orders',
                'import_orders', 'export_collections', 'print_orders', 'print_collections',
                'edit_orders', 'create_businesses'
            ];
            ...
            const res = await fetch(`${process.env.EXPO_PUBLIC_API_URL}${url}`, {
                ...
                body: JSON.stringify({
                    ...
                    permissions: roleId === 2 ? defaultBusinessPermissions : undefined,
                    country: "palestine",
            ...
            setShowAlert({
                ...
                onClose: () => {canManageUsers ? router.push("(users)") : router.push("(tabs)")}
            });
            ...
            setSelectedValue((selectedValue) => (
                {
                    ...
                    role_id: { name: userData.role, role_id: userData.role_id },
                    branch_id: { name: userData.branch, branch_id: userData.branch_id },
                    pricelist_id: { name: userData.priceList, pricelist_id: userData.priceList_id }
                }
            ));
            setForm({
                name: userData.name,
