...
export default function Settings() {
    ...
    const settings = {
        preferences: [
            (["accountant","support_agent","entery","admin","manager","business"].includes(user?.role)) ? {
                label: translations[language].tabs.settings.options.users,
                onPress: () => router.push("(users)"),
                icon: <FontAwesome name="user-o" size={22} color="#4361EE" />
            } : null,
            (["admin","manager","support_agent","driver","sales_representative"].includes(user?.role)) ? {
                label: translations[language].tabs.settings.options.sales_clients,
        ...
        <View style={[styles.container, { backgroundColor: colors.background }]}>
            ...
            {showAccountsModal && (
                ...
                >
                    ...
                    <ScrollView
                        style={styles.accountsModalScroll}
                        contentContainerStyle={styles.accountsModalScrollContent}
