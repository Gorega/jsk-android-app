    ...
    try {
      ...
      const [moneyRes, packageRes] = await Promise.all([
        axios.get(`${process.env.EXPO_PUBLIC_API_URL}/api/collections`, {
          params: { type_id: 4, status: 'handed_over|money_out' },
          headers: {
            'Accept': 'application/json',
            'Content-Type': 'application/json',
          },
                ...
                onPress={() => {
                  ...
                  const statusKey = index === 0 ? "today" :
                    index === 1 ? "money_in_branch" :
                      index === 2 ? "returned_in_branch,returned_received_in_branch" :
                        index === 3 ? "sent_money" :
              ...
              >
                <View style={[
                  ...
                  <View style={styles.cardContent}>
                    ...
                    {user.role === "business" && (index === 1 || index === 2) && (
                      <TouchableOpacity
                        style={styles.cardThreeDotsAction}
                        onPress={() => {
                          if (index === 1) handleMoneyLongPress();
                          else if (index === 2) handlePackageLongPress();
                        }}
                      ...
                      >
                        <Text style={[styles.cardActionText, {
                          ...
                          {translations[language]?.collections?.collection?.action}
                        </Text>
